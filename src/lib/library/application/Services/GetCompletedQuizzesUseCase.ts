...
export class GetCompletedQuizzesUseCase {
  ...
  async run(id: UserIdDTO, queryInput:QueryParamsInput): Promise<Either<HttpException, QueryResponse<PlayingQuizResponse>>> {
    ...
    const quizzesIds = inProgressGames.map(quiz => QuizId.of(quiz.getQuizId().value));
    ...
    for(let i = 0; i < inProgressGames.length; i++){
      ...
      const quiz = quizzes.find(q => q.id.value === inProgressGame.getQuizId().value);
      if(quiz){
        const author = quizAuthors.find(u => u.id.value === quiz.authorId.value);
        if(author){
          const playingQuizResponse = toPlayingQuizResponse(quiz, author, inProgressGame, "singleplayer");
    ...
    return Either.makeRight<HttpException, QueryResponse<PlayingQuizResponse>>(answer);
  }
}