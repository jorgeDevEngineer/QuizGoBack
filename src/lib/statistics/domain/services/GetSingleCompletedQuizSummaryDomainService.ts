...
export class GetSingleCompletedQuizSummaryDomainService {
  constructor(
  ...
  public async execute(
    ...
    const completedGame =
      await this.singlePlayerGameRepository.findById(gameId);

    if (!completedGame) {
      return Either.makeLeft(
        new GameNotFoundException("No se ha encontrado al partida solicitada.")
      );
    }
